...
def getCluster():
    ...
    minibatchkmeans = MiniBatchKMeans(n_clusters=600).fit(data)
    ...
    for key in cluster.keys():
        ...
        cmd = "echo *'{0}'>>{1}".format(key,"cluster_600c")
        os.system(cmd)
        
        cmd = "echo *'{0}'>>{1}".format(key,"VecCluster_600c")
        os.system(cmd)

        i = 0 
        for apk in clusterList:
            cmd = "echo {0}>>{1}".format(apk,"cluster_600c")
            os.system(cmd)
            cmd = "echo {0}>>{1}".format(vectorList[i],"VecCluster_600c")
            os.system(cmd)
            i = i+ 1
    ...
    cmd = "echo {0}>>{1}".format(elpase,'timeRecord')
    os.system(cmd)
