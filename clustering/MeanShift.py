...
def getCluster():
    ...
    for key in cluster.keys():
        ...
        cmd = "echo *'{0}'>>{1}".format(key,"cluster_1300c")
        os.system(cmd)
        
        cmd = "echo *'{0}'>>{1}".format(key,"VecCluster_1300c")
        os.system(cmd)

        i = 0 
        for apk in clusterList:
            cmd = "echo {0}>>{1}".format(apk,"cluster_1300c")
            os.system(cmd)
            cmd = "echo {0}>>{1}".format(vectorList[i],"VecCluster_1300c")
            os.system(cmd)
            i = i+ 1
