...
def getCluster():
    ...
    ac = AgglomerativeClustering(n_clusters=1100).fit(data)
    ...
    for key in cluster.keys():
        ...
        cmd = "echo *'{0}'>>{1}".format(key,"cluster1100")
        os.system(cmd)
        
        cmd = "echo *'{0}'>>{1}".format(key,"VecCluster1100")
        os.system(cmd)

        i = 0 
        for apk in clusterList:
            cmd = "echo {0}>>{1}".format(apk,"cluster1100")
            os.system(cmd)
            cmd = "echo {0}>>{1}".format(vectorList[i],"VecCluster1100")
            os.system(cmd)
            i = i + 1
