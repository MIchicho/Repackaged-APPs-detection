...
def getCluster():
    ...
    sp = SpectralClustering(affinity='nearest_neighbors',n_neighbors=10,n_clusters=400).fit(data)
    ...
    for key in cluster.keys():
        ...
        cmd = "echo *'{0}'>>{1}".format(key,"cluster_400c")
        os.system(cmd)
        
        cmd = "echo *'{0}'>>{1}".format(key,"VecCluster_400c")
        os.system(cmd)

        i = 0 
        for apk in clusterList:
            cmd = "echo {0}>>{1}".format(apk,"cluster_400c")
            os.system(cmd)
            cmd = "echo {0}>>{1}".format(vectorList[i],"VecCluster_400c")
            os.system(cmd)
            i = i+ 1
