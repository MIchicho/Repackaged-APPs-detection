...
def getCluster():
    ...
    af = AffinityPropagation(damping=0.8).fit(data)
    ...
    for key in cluster.keys():
        ...
        cmd = "echo *'{0}'>>{1}".format(key,"cluster_damp08")
        os.system(cmd)
        
        cmd = "echo *'{0}'>>{1}".format(key,"VecCluster_damp08")
        os.system(cmd)

        i = 0 
        for apk in clusterList:
            cmd = "echo {0}>>{1}".format(apk,"cluster_damp08")
            os.system(cmd)
            cmd = "echo {0}>>{1}".format(vectorList[i],"VecCluster_damp08")
            os.system(cmd)
            i = i+ 1
