...
def getCluster():
    ...
    fa = FeatureAgglomeration(n_clusters=50).fit(data)
    ...
    for key in cluster.keys():
        ...
        cmd = "echo *'{0}'>>{1}".format(key,"cluster")
        os.system(cmd)
        
        cmd = "echo *'{0}'>>{1}".format(key,"VecCluster")
        os.system(cmd)

        i = 0 
        for apk in clusterList:
            cmd = "echo {0}>>{1}".format(apk,"cluster")
            os.system(cmd)
            cmd = "echo {0}>>{1}".format(vectorList[i],"VecCluster")
            os.system(cmd)
            i = i+ 1
