...
#              5. we should delete some graph only have one graph 
...
# 注意边要遍历干净，用深度遍历比较好
# 注意一些边的特殊情况
# sha256: B8B15A52DFC953B2A5E448A8A762EBEE190E0E425C53325D32E78058C65DBE34
...
'''
# we should pay attention to that some children and parents have in the EdgeCollection
# edgeDict the relationship between parents and children 
...
if (input == '0'):
    ...
    outputPath = "/home/chicho/workspace/repackaged/ViewDroid/experiment/casestudy/originalANALYSIS.txt"
    graphPath = "/home/chicho/workspace/repackaged/ViewDroid/experiment/casestudy/original/"
...
if (input == '1'):
    ...
    outputPath = "/home/chicho/workspace/repackaged/ViewDroid/experiment/casestudy/repackagedANALySIS.txt"
    graphPath = "/home/chicho/workspace/repackaged/ViewDroid/experiment/casestudy/repackaged/"
...
def getTheEdgeSet(manifestPath,apkSatgPath,apkname):
    ...
    cmd = '''echo "**********************" >> {0}'''.format(outputPath)
    os.system(cmd)

    cmd = "echo 'we are handle the apk {0}' >>{1}".format(apkname,outputPath)
    os.system(cmd)

    cmd = '''echo {0}>>{1}'''.format(apkname,outputPath)
    os.system(cmd)
    ...
    cmd = '''echo "package Name: " {0}>>{1} '''.format(package,outputPath)
    os.system(cmd)

    cmd = '''echo "activity count: " {0} >>{1}'''.format(actCnt,outputPath)
    os.system(cmd)
...
#                        print tmpParentNameList
                        ...
                        for child in childActList:
                            ...
                            if child in tmpParentNameList:
                                ...
                                if parentName not in splitParent:
                                    ...
                                    grandchildActList = edgeDict[parentName]

                                    for grandChild in grandchildActList:
                                        if not grandChild in EdgeCollection:
                                            EdgeCollection.append(grandChild)
                                            splitEdgeCollection.append(grandChild)
                ...
                print "****************"
                print "**************88EdgeCollection**********"

                print EdgeCollection
                
                print "+++++++splitEdgeCollection+++++++"
                print splitEdgeCollection
                ...
                for parentName in splitParent:
                    if parentName in EdgeCollection:

                        if parentName in tmpParentNameList:
                            tmpParentNameList.remove(parentName)
                        ...
                        for child in childActList:
                            ...
                            if child in tmpParentNameList:
                                ...
                                if child not in splitParent:
                                    splitParent.append(child)
                                    tmpParentNameList.remove(child)

                                grandchildActList = edgeDict[grandParent]

                                for grandChild in grandchildActList:
                                    if grandChild not in EdgeCollection:
                                        EdgeCollection.append(grandChild)
                                        splitEdgeCollection.append(grandChild)
                                
                print "***************"
                print "we are in interator"
                ...
                # child in the EdgeCollection but parent not in EdgeCollection parent in splitParent or tmpParentNameList
                # special example: original:6258ECEB9D58EBDB238
                ...
                tmpParentNameList2 = tmpParentNameList
                for j in range(len(sortWeight)+1):
                    ...
                    for parentName in tmpParentNameList:   
                        ...
                        print "we are in the children in the EdgeCollection but parent not"
                        ...
                        for child in childActList:
                            if child in EdgeCollection:
                                ...
                                print "child in the EdgeCollection"
                                print child 
                                print splitEdgeCollection
                ...
                for k in range(tmpPCnt):
                    ...
                    for parentName in tmpParentNameList:
                        ...
                        if parentName in EdgeCollection:
                            ...
                            if not parentName in splitEdgeCollection:
                                splitEdgeCollection.append(parentName)
        ...
        cmd = '''echo "t"{0} >>{1}'''.format(i,outputPath)
        os.system(cmd)
        ...
        for key in splitParent:
            ...
            for child in tmpList:
                ...
                sourceALL = EdgeCollection.index(sourceName)
                targetALL = EdgeCollection.index(targetName)
                ...
                try:
                    ...
                    cmd1 = '''echo "e " {0} " --> " {1} >> {2}'''.format(sourceName.encode('utf-8'),targetName.encode('utf-8'),outputPath)
                    os.system(cmd1)
                ...
                cmd3 = '''echo "e " {0} " --> " {1} >> {2}'''.format(sourceALL,targetALL,outputPath)
                os.system(cmd3)
...
if __name__ == '__main__':
    ...
    pairPath = "/home/chicho/workspace/repackaged/ViewDroid/experiment/casestudy/caseStudy.txt"
    ...
    for apk in APKList:
        ...
        # invoke the function 
        getTheEdgeSet(manifestPath,apkSatgPath,apk)
