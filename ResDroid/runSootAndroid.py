...
if ( input == '0' ):
    ...
    gator_path = "/home/chicho/result/APK_17_COMM/"
    outPutpath = "/home/chicho/result/sootAndroidOut_17_COMM/"
else:
...
outList = os.listdir(outPutpath)
guiList = []
for guifile in outList:
    if guifile.endswith(".xml"):
        guiname = guifile.split("-")
        guiname.pop()
        guiname = '-'.join(guiname)
        guiList.append(guiname)
...
for project in GatorProjPath:
    ...
    if project in guiList:
        print "project has processed"
        continue 
    Projpath = os.path.join(gator_path,project)
    SootAndroidPath = "/home/chicho/workspace/SootAndroid/scripts/guiAnalysis.sh"    
